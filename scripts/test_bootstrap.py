#!/usr/bin/env python3
"""Exercise the real bootstrap twice in an isolated copy."""
...
def assert_generated(root: Path) -> None:
    ...
    if "@Brak23" not in codeowners:
        raise AssertionError("generated CODEOWNERS is incomplete")
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="codex-template-") as temporary:
        ...
        command = [sys.executable, "scripts/bootstrap_project.py", "--config", "project.config.example.yml"]
        run(command, copy)
